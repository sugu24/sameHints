...
def sameHint(datas, hintStr_file):
    hint_dict = dict()
    i = 0
    while i < len(datas):
...
def writeFile(sameHintStr_file, hint_dict):
    same = []
...
datas = readFile(input_directory+"sudoku.txt")
for W in range(3, 11):
    for L in range(2, W+1):
        file_id = str(W) + "_" + str(L)
        sudoku_file = input_directory + "sudoku" + file_id + ".txt"
        hintStr_file = output_directory + "hintStr___" + file_id + ".txt"
        sameHintStr_file = output_directory + "sameHintStr_" + file_id + ".txt"
        hint_dict = sameHint(datas[(W, L)], hintStr_file)
        print(len(hint_dict))
        writeFile(sameHintStr_file, hint_dict)