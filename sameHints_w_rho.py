...
max_process = 20
w, rho = int(sys.argv[1]), int(sys.argv[2])
output_id = str(sys.argv[1]) + "_" + str(sys.argv[2])
input_directory = "hints/"
output_directory = "result/"
sudoku_file = input_directory + "sudoku.txt"
hintStr_file = output_directory + "hintStr_" + output_id + ".txt"
sameHintStr_file = output_directory + "sameHintStr_" + output_id + ".txt"
...
def readFile(file_name, w, rho):
    datas = []
    with open(file_name, "r") as f:
        for d in f.readlines():
            d = list(map(str, d.split()))
            if int(d[3]) != 17: continue
            if int(d[0]) != w or int(d[1]) != rho or int(d[2]) != 1:
                continue
            datas.append(d)
    print(w, rho, 1, " is ", len(datas))
    return datas
...
datas = readFile(sudoku_file, w, rho)
sameHint()
print(len(hint_dict))
writeFile()
